...
export class BaseApi {
  ...
  connection(method = 'GET', url, body, fileList) {
    ...
    if(Auth.getLogin()){
      this.servers.defaults.headers.common['partyId'] = Auth.getUserInfo().partyId;
      this.servers.defaults.headers.common['account'] = Auth.getUserInfo().account;
      this.servers.defaults.headers.common['token'] = Auth.getUserInfo().token;
    }
    // this.getStatusToken();
...
export const server = new BaseApi("http://localhost:9090");
